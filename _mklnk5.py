...
bat = os.path.join(project, "启动VisionLoop.bat")
...
ps_path = os.path.join(os.environ["TEMP"], "vl-setup5.ps1")
...
if os.path.exists(shortcut):
    with open(shortcut, "rb") as f:
        data = f.read()
    if "启动VisionLoop.bat".encode("utf-16-le") in data:
        print("OK - shortcut points to bat correctly")
    else:
        # check what the target actually is
        idx = data.find(b".bat")
        if idx > 0:
            chunk = data[idx-80:idx+10]
            print(f"Bat nearby: {chunk}")
        else:
            print("No .bat reference found")
else:
    print("FAILED")